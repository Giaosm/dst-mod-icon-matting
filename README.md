...
| Python | **无需提前安装**：系统有 3.10+ 就直接用（优先找 Homebrew 装的）；没有的话引导会自动用 **Homebrew** 安装 Python 3.12（`brew install python@3.12`），不影响系统其它 Python |
...
**全程不需要自己安装 Python**——见第 6 步，引导会自动搞定。
...
### 第 6 步 · 等待自动完成「准备 Python + 装依赖 + 下权重」

引导会自动做三件事，**全程无需操作**：

1. **准备 Python**：系统里已有 Python 3.10+ 就直接用（优先找 Homebrew 装的）；
   没有的话会**自动用 Homebrew 安装 Python 3.12**（`brew install python@3.12`，
   符合 macOS 惯例，装好即永久可用）。没有 Homebrew 时终端会给出安装命令。
   全程不需要你自己装 Python；
2. **安装依赖**：自动创建虚拟环境 `.venv`，只装所选模型需要的依赖（已装过的秒跳过）；
3. **下载模型权重**（约 0.2 ~ 1 GB）：有进度显示。中途断网或取消没关系，
   **重新运行会自动断点续传**，不会从头再来。

> 若终端出现 `检测到本地代理`，只是下载时走了本机代理加速，属正常提示，不影响之后使用。
> 若下载 Python 那步因网络失败，终端会给出手动安装命令，装好重跑即可（极少见）。
...
不用担心：引导发现系统没有 Python 3.10+ 时，会**自动用 Homebrew 安装 Python 3.12**
（`brew install python@3.12`，符合 macOS 惯例）；没有 Homebrew 则终端会给出手动安装命令。
全程无需手动安装（见部署第 6 步）。
...
看具体报错，装好后重跑 `./setup.sh` 即可。国内网络若 Homebrew 本身很慢，可先设置
清华镜像加速（`HOMEBREW_API_DOMAIN` / `HOMEBREW_BOTTLE_DOMAIN` 指向
`https://mirrors.tuna.tsinghua.edu.cn/...`），或换用官网 python.org 安装包。
...
注意这个手动方式需要**系统里本来就有 Python 3.10+**；没有的话直接用 `./run.sh` 自动方式即可。）
